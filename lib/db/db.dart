...
class DB {
  static Future<bool> databaseExists() async => databaseFactory.databaseExists(join(await getDatabasesPath(), 'demo.db'));
  ...
  static Future<Database> _openDB() async {
    ...
    return openDatabase(
      ...
      onCreate: (Database db, int version) async {
        ...
        return db.execute('CREATE TABLE usuario (id INTEGER PRIMARY KEY, name TEXT, genre TEXT, score INTEGER)');
      }
    );
  }
  ...
  static Future<void> update(Usuario usuario) async{
    Database db = await _openDB();
    db.update('usuario', usuario.toMap(), where: 'id = ?', whereArgs: [usuario.id]);
  }
  static Future<List<Usuario>> select() async{
    Database db = await _openDB();
    List<Map<String, dynamic>> usuarioList = await db.query('usuario', limit: 1);
    return
    List.generate(
      usuarioList.length,
      (i) => Usuario(
